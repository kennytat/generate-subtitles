  ...
  try {
    ...
    for (const file of files) {
      if (file.timestamp > withinLastHour) {
        stats.lastHour++;
        transcriptionTime.lastHour +=
          file.processingData.uploadDurationInSeconds;
      }
      if (file.timestamp > within24h) {
        stats.last24h++;
        transcriptionTime.last24h +=
          file.processingData.uploadDurationInSeconds;
      }
      if (file.timestamp > withinWeek) {
        stats.lastWeek++;
        transcriptionTime.lastWeek +=
          file.processingData.uploadDurationInSeconds;
      }
      if (file.timestamp > withinMonth) {
        stats.lastMonth++;
        transcriptionTime.lastMonth +=
          file.processingData.uploadDurationInSeconds;
      }
      stats.allTime++;
    ...
    transcriptionTime.lastHour = forHumansHoursAndMinutes(
      transcriptionTime.lastHour
    );
    transcriptionTime.last24h = forHumansHoursAndMinutes(
      transcriptionTime.last24h
    );
    transcriptionTime.lastWeek = forHumansHoursAndMinutes(
      transcriptionTime.lastWeek
    );
    transcriptionTime.lastMonth = forHumansHoursAndMinutes(
      transcriptionTime.lastMonth
    );
    transcriptionTime.allTime = forHumansHoursAndMinutes(
      transcriptionTime.allTime
    );
