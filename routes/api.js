...
const { languagesToTranslateTo, newLanguagesMap, translationLanguages } =
  constants;
...
router.post("/api", upload.single("file"), async function (req, res, next) {
  try {
    console.log("/api called::", req, res);
    // fix body data
    ...
    const { model, language, downloadLink, apiToken, websocketNumber } =
      postBodyData;
    ...
    const authTokenString = await fs.readFile(
      `${process.cwd()}/constants/apiTokens.txt`,
      "utf8"
    );
    ...
    if (!downloadLink && !file) {
      ...
      return res
        .status(400)
        .json({ error: `Please pass either a 'file' or 'downloadLink'` });
    }

    // bad model name
    if (!validModelValues.includes(model)) {
      return res.status(400).send({
        error: `Your model of '${model}' is not valid. Please choose one of the following: ${validModelValues.join(
          ", "
        )}`,
      });
    }
    ...
    if (!whisperLanguagesHumanReadableArray.includes(language)) {
      return res.status(400).send({
        error: `Your language of '${language}' is not valid. Please choose one of the following: ${whisperLanguagesHumanReadableArray.join(
          ", "
        )}`,
      });
    }
    ...
    await fs.mkdirp(path.join(transcriptionDir, numberToUse));

    const newPath = path.join(transcriptionDir, numberToUse, numberToUse);
    ...
    if (downloadLink) {
      ...
      uploadFilePath = path.join(uploadDir, matchingFile);
    } else {
      res.send({
...
router.get("/api/:sdHash", async function (req, res, next) {
  try {
    ...
    const processingData = JSON.parse(
      await fs.readFile(
        path.join(transcriptionDir, sdHash, "processing_data.json"),
        "utf8"
      )
    );
    ...
    if (
      ...
      const originalVtt = await fs.readFile(
        path.join(transcriptionDir, sdHash, `${sdHash}.vtt`),
        "utf8"
      );
      subtitles.push({
        language,
      ...
      //   const originalVtt = await fs.readFile(path.join(transcriptionDir,sdHash, `${sdHash}_${translatedLanguage}.vtt`), 'utf8');
      //   subtitles.push({
