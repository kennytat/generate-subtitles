...
router.post("/file", upload.single("file"), async function (req, res, next) {
  console.log("onFileSubmit::", req.body);
  console.log("testSubmittedFile::", path.resolve(req.file.originalname));
  // l(global.ws);
  ...
  try {
    ...
    if (passedFile) {
      ...
      l("filename");
      l(filename);
      uploadGeneratedFilename = path.parse(filename).name;
      originalFileNameWithExtension = filename;
      const baseName = path.parse(filename).name;
      const extension = path.parse(filename).ext;
      uploadedFilePath = path.join(uploadDir, `${randomNumber}${extension}`);
    ...
    if (isFreeSubtitles && !isYtdlp) {
      const amountOfSecondsInHour = 60 * 60 * 3;
    ...
    const totalOutstanding =
      currentlyRunningJobs + amountInQueue - maxConcurrentJobs + 1;
