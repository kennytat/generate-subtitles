...
function handleStdErr({
  ...
  return function (data) {
    (async function () {
      ...
      const { percentDoneAsNumber } = formattedProgress;
...
async function moveFiles(randomNumber, fileExtension) {
  const holderFolder = path.join(tmpDir, "transcriptions", randomNumber);
...
//   path: path.join(uploadDir, '572fa0ecb660b1d0eb489b879c2e2310'),
//   size: 22904865
// }
...
const makeFileNameSafe = function (string) {
  return filenamify(string, { replacement: "_" }) // replace all non-URL-safe characters with an underscore
    .split("：")
    .join(":") // replace chinese colon with english colon
...
function createFileNames(originalFileName) {
  ...
  return {
    ...
    directorySafeFileNameWithoutExtension: makeFileNameSafe(
      originalFileNameWithoutExtension
    ), // safe file name for directory name
  };
}
