...
function deleteFromGlobalTranscriptionsBasedOnWebsocketNumber(websocketNumber) {
  ...
  const closerTranscription = global["transcriptions"].find(function (
    transcription
  ) {
...
function deleteWebsocketAndEndProcesses({
  websocketNumber,
  websocketConnection,
  websocket,
  index,
...
function checkForDeath() {
  ...
  l(`Total current connection:: ${totalAmountOfWebsockets}`);
  ...
  for (let [index, websocket] of global["webSocketData"].entries()) {
    ...
    if (websocketConnection.isAlive === false) {
      deleteWebsocketAndEndProcesses({
        websocketNumber,
        websocketConnection,
        websocket,
        index,
