...
async function transcribe({
  ...
  return new Promise(async (resolve, reject) => {
    function webSocketIsStillAlive(webSocketNumber) {
      return global.webSocketData.some(
    ...
    try {
      ...
      let uploadFolderFileName = uploadedFilePath
        .split(osSpecificPathSeparator)
        .pop();

      const originalUpload = path.join(uploadDir, uploadFolderFileName);
      console.log(
        "logPath::",
        uploadedFilePath,
        originalUpload,
      ...
      const uploadDurationInSecondsHumanReadable = forHumans(
        uploadDurationInSeconds
      );
      ...
      whisperProcess.on("close", async (code) => {
        try {
          ...
          if (processFinishedSuccessfully) {
            ...
            const originalContainingDir = path.join(
              transcriptionDir,
              uploadGeneratedFilename
            );

            const originalDirectoryAndNewFileName = path.join(
            ...
            const { strippedText, timestampsArray } =
              await stripOutTextAndTimestamps(vttPath);
            ...
            const processingSeconds = Math.round(
              (new Date() - startingDate) / 1000
            );

            const processingRatio = (
              uploadDurationInSeconds / processingSeconds
            ).toFixed(2);
            ...
            uploadDurationInSecondsHumanReadable: ${forHumans(
              uploadDurationInSeconds
            )}
          ...
          `.replace(/^ +/gm, ""); // remove indentation
            ...
            if (translationStartedAndCompleted) {
              ...
              translatedLanguages = languagesToTranscribe.filter(
                (e) => e !== language
              );
            }
            ...
            const wordsPerMinute = Math.round(
              wordCount / (uploadDurationInSeconds / 60)
            );
            ...
            await fs.appendFile(
              path.join(originalContainingDir, "processing_data.json"),
            ...
            removeFromArrayByWebsocketNumber(
              global.transcriptions,
              websocketNumber
            );
          } else {
...
async function createZipArchive(input, output) {
  return new Promise(async function (resolve, reject) {
    ...
    fileObjs = fs.readdirSync(input);
    for await (const file of fileObjs) {
      if (/(srt|vtt|txt|json|tsv|csv)$/.test(path.extname(file))) {
        zip.addLocalFile(path.join(input, file));
      }
    }
    zip.writeZip(output);
    resolve("done");
